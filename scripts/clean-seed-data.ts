 ...
 *
 * Run: DATABASE_URL="..." npx tsx scripts/clean-seed-data.ts
 */
...
async function main() {
  ...
  // 1. Delete fake users (keep only admin, tomanderson, johndoe)
  const deletedUsers = await prisma.user.deleteMany({
    where: { username: { notIn: ESSENTIAL_USERS } },
  })
  console.log(`Deleted ${deletedUsers.count} fake users`)

  // 2. Reset stats on essential users to zero
